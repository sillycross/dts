...
if(!isset($cmd)){
	...
	if($name_check!='name_ok'){
		...
		if($db->num_rows($result) > 0) {
			...
			$result = $db->query("INSERT INTO {$gtablepre}users (username,password,groupid,ip,credits,gender,icon,motto,killmsg,lastword,gold,cardlist) VALUES ('$username', '$stored_password', '$groupid', '$onlineip', '$credits', '$gender', '$icon', '$motto', '$killmsg', '$lastword','80','0')");
			if($result){
