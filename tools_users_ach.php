...
while($ud = $db->fetch_array($result)){
	...
	$add = 0;
	if($a[353] >= 3 && $a[353] < 5) {
		echo '账户 '.$ud['username'].' 缺失第一阶段奖励<br>';ob_end_flush(); flush();
		$add += 1333;
	}
	
	if($a[353] >= 6 && $a[353] < 12) {
		echo '账户 '.$ud['username'].' 缺失第二阶段奖励<br>';ob_end_flush(); flush();
		$add += 2333;
	}
	if($add) reissue($ud['username'], $add);
}
echo 'done.';

function reissue($un, $q, $ai=0){
	$pt = '因为「翻盘神器」系列成就获得条件修改，您已完成该成就，现补发奖励。';
