...
function showData(sdata){
	...
	if(shwData['url']) {
		if (shwData['url'] == 'error.php')	//gexit error
		{
			var form = jQuery('<form action="error.php" name="errorpost" method="post" style="display:none;"><input type="text" name="errormsg" value="' + shwData['errormsg'] + '" /></form>');
			jQuery('body').append(form); form.submit();
		}
		else  window.location.href = shwData['url'];
