...
namespace skill469
{
	...
	function parse_news($nid, $news, $hour, $min, $sec, $a, $b, $c, $d, $e, $exarr = array())
	{
		...
		if($news == 'death40') {	
			...
			return "<li id=\"nid$nid\">{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a</span>受到<span class=\"yellow\">$c</span>的袭击，自爆身亡了！{$e0}";
		}
		if($news == 'death41') {
			$dname = $typeinfo[$b].' '.$a;
			if(!$e)
			...
			return "<li id=\"nid$nid\">{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a</span>被<span class=\"yellow\">$c</span>的自爆炸死了！{$e0}";
		}
