...
namespace skill523
{
	...
	function init()
	{
		define('MOD_SKILL523_INFO','card;unique;');
	...
	function check_skill523_proc(&$pa, &$pd, $active){
		...
		if (\skillbase\skill_query(523,$pd) && check_unlocked523($pd) && (strstr($pa['wepk'], 'F') == '')){
			$log .=  \battle\battlelog_parser($pa, $pd, $active, '<span class="yellow b">对太阳的妖精的物理伤害无效！<br>你无法用物理攻击刺穿这个角色的皮肤！</span><br>');
			$r = 1;
			return $r;
		}
		return 0;
