...
namespace skill526
{
	...
	function check_skill526_proc(&$pa, &$pd, $active, $dmg){
		...
		elseif (!$active && (strstr($pd['wepk'], 'F') != '')){
			$log .=  \battle\battlelog_parser($pa, $pd, $active, '<span class="yellow b"><:pd_name:>的物理伤害无效</span><br>');
			return 1;
		}

		if ($active && ((strstr($pa['wepk'], 'K') != '') || (strstr($pa['wepk'], 'P') != '') || (strstr($pa['wepk'], 'N') != ''))) {
			$rate = 0.3;
			$pd['att'] += $rate * $dmg;
		}
		elseif (!$active && ((strstr($pd['wepk'], 'K') != '') || (strstr($pd['wepk'], 'P') != '') || (strstr($pd['wepk'], 'N') != ''))) {
			$rate = 0.2;
			$pa['att'] += $rate * $dmg;
			//玩家部分暂定
		}
