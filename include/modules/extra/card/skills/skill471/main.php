...
namespace skill471
{
	...
	function apply_total_damage_modifier_insurance(&$pa,&$pd,$active)
	{
		...
		if (\skillbase\skill_query(471,$pd) && $pa['dmg_dealt']>=$pd['hp'] && $pd['rage']>0)
		{
			...
			if ($active)
				$log.="<span class=\"yellow b\">敌人发动气功，消耗{$rageused}点怒气抵挡了<span class=\"red b\">{$dmgred}</span>点伤害！</span><br>";
			else  $log.="<span class=\"yellow b\">你发动气功，消耗{$rageused}点怒气抵挡了<span class=\"red b\">{$dmgred}</span>点伤害！</span><br>";
			$pd['rage']-=$rageused;
			$pa['dmg_dealt']-=$dmgred;
		}
	}
