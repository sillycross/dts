...
示例：
	...
	“火焰属性”的实现：（临时性技能）
		...
		接管skill_onload_event函数，判断此人是否有火焰属性武器，如果有，调用\skillbase\skill_acquire(123,$pa)
		
		接管战斗模块里对应函数，如果获得了技能就执行对应的任务
		
		接管skill_onsave_event这个函数，判断如果此技能存在则失去此技能（调用\skillbase\skill_lost(123,$pa)）
		
		这是因为“火焰属性”是非持久的（卸下武器就没了），所以在载入玩家时候判定获得，保存玩家时候自动失去
		（如果不失去就会出现即使玩家卸下了武器或因其他原因武器消失了也会保留火焰属性的情况，
		当然一个等价的做法是接管所有可能安装/卸下武器的函数去判，但这是不可行的，因为需要接管的东西太多了，故不采用）
