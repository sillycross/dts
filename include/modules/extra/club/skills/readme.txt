...
club 		无		表明这个技能是一个称号技能。所有称号技能都必须带上这个标签
...
hidden	基本特性	这个技能不会在技能页面中显示，一般用于称号的基本特性。
				例如“开局获得25殴系熟练，升级增加殴熟”这种技能就应该加这个标签。
				有这个标签的技能自动视为locked。
				
name		名字		如果既没有locked标签也没有hidden标签则这个标签必须存在，代表技能的名字。格式是name=名字;

假如你的称号技能在技能页面中有“升级”以外的可用操作（或者你的操作有特殊需要），
请一律将变量设置为mode='special' command='skill编号_special' subcmd='你的操作'，
然后接管\player\act()进行处理（记住subcmd和用户提交的其他变量需要import_module('input')才能使用）。

提供了5个html变量标签，分别为skillpara1,skillpara2,skillpara3,skillpara4,skillpara5;
有需要的可以自己在按钮处用$('skillpara1').value=XXX;赋值作为参数，然后处理函数里import_module('input')就能用了。
请不要再自己开新的html变量标签了。
...
同时，所有非hidden称号技能都应当提供一个函数check_unlocked编号(&$pa)，返回该技能是否已经被解锁（即是否可以使用了）
hidden技能应该全部默认开局解锁。
