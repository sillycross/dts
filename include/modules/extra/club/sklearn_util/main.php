				...
				for ($i=0; $i<$rz; $i++)
				{
					...
					for ($j=0; $j<$cz; $j++)
					{
						...
						else
						{
							...
							echo '<td class="b3"><span style="width:40px;" onmouseover="$(\'skl_util_'.$caller_id.'_skilllearn_'.$skillid.'\').style.top=jQuery(this).offset().top+10; $(\'skl_util_'.$caller_id.'_skilllearn_'.$skillid.'\').style.left=jQuery(this).offset().left+20'.($j<3?'':'-420').'; $(\'fmsgbox-container\').appendChild($(\'skl_util_'.$caller_id.'_skilllearn_'.$skillid.'\')); $(\'skl_util_'.$caller_id.'_skilllearn_'.$skillid.'\').style.display=\'block\';" onmouseout="$(\'skl_util_'.$caller_id.'_skilllearn_'.$skillid.'\').style.display=\'none\';"><input type="button" style="width:38px" value="查看"></span>';
