...
<?php } else { echo '___aaf1'; } ?><?php $clv = (int)\skillbase\skill_getvalue(32,'lvl');  $totlv = count($upgradecost)-1;  ?>
...
持殴系武器时，武器效果值的<span class="yellow"><?php } else { echo '___aahr'; } ?><?php echo $defgain[$clv]?><?php if (!defined('GEXIT_RETURN_JSON')) { ?>%</span>计入防御力(最多2000点)<br>
</span></td>
<td class=b3 width=46>
<?php } else { echo '___aahs'; } ?><?php if($upgradecost[$clv] !== -1) { if($skillpoint>=$upgradecost[$clv]) { ?>
