...
<?php } else { echo '___aaht'; } ?><?php include template('MOD_CLUBBASE_SKILLDESC_MID1'); ?>
...
持投系武器方可发动，消耗<span class="yellow"><?php } else { echo '___aahu'; } ?><?php echo $ragecost?><?php if (!defined('GEXIT_RETURN_JSON')) { ?></span>点怒气
</span></td>
<td class=b3 width=46>
<!--这个只是一个假的按钮，实际点击的是下面那个-->
<?php } else { echo '___aahv'; } ?><?php if(!$___TEMP_THIS_SKILL_ACQUIRED) { if($skillpoint>=$unlockcost) { ?>
...
消耗<span><?php } else { echo '___aahy'; } ?><?php echo $unlockcost?><?php if (!defined('GEXIT_RETURN_JSON')) { ?></span>点技能点解锁
<span style="float:right;margin-right:1px;">
<?php } else { echo '___aahz'; } ?><?php if($skillpoint>=$unlockcost) { ?>
...
<?php } else { echo '___aahA'; } ?><?php } else { ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?><input type="button" style="width:47px" disabled="true" value="解锁">
<?php } else { echo '___aahB'; } ?><?php } ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?></span>
</span>
<?php } else { echo '___aahC'; } ?><?php } include template('MOD_CLUBBASE_SKILLDESC_END'); ?>
