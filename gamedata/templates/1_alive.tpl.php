...
<input type="button" name="enter" value="显示前<?php } else { echo '___aao-'; } ?><?php echo $alivelimit?><?php if (!defined('GEXIT_RETURN_JSON')) { ?>名幸存者" onClick="$('alivemode').value='last';postCmd('alive','alive.php');">
<input type="button" name="enter" value="显示全部幸存者" onClick="$('alivemode').value='all';postCmd('alive','alive.php');">
</form>
<div id="alivelist">
<?php } else { echo '___aapa'; } ?><?php include template('alivelist'); ?>
...
<?php } else { echo '___aapb'; } ?><?php include template('footer'); ?>
