...
<?php } else { echo '___aagk'; } ?><?php include template('MOD_CLUBBASE_SKILLDESC_MID1'); ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?><td class=b3><span style="margin-top:8px; margin-bottom:8px; display:block;">
<?php } else { echo '___aafU'; } ?><?php $clv=(int)\skillbase\skill_getvalue(52,'lvl'); $totlv = count($upgradecost)-1;  ?>
...
手持投系武器时，反击率<span class="yellow">+<?php } else { echo '___aagl'; } ?><?php echo $counterperc[$clv]?><?php if (!defined('GEXIT_RETURN_JSON')) { ?>%</span></td>
<td class=b3 width=46>
<?php } else { echo '___aagm'; } ?><?php if($upgradecost[$clv] !== -1) { if($skillpoint>=$upgradecost[$clv]) { ?>
...
<?php } else { echo '___aagn'; } ?><?php } else { ?>
