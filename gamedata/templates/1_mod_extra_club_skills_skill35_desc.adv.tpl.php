...
<?php } else { echo '___aaf0'; } ?><?php include template('MOD_CLUBBASE_SKILLDESC_MID1'); ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?><td class=b3><span style="margin-top:8px; margin-bottom:8px; display:block;">
<?php } else { echo '___aaf1'; } ?><?php $clv = (int)\skillbase\skill_getvalue(35,'lvl');  $totlv = count($upgradecost)-1;  ?>
...
持殴系武器战斗时<span class="yellow"><?php } else { echo '___aaf6'; } ?><?php echo $proc_rate?><?php if (!defined('GEXIT_RETURN_JSON')) { ?>%</span>几率触发，触发则物理伤害增加<span class="yellow"><?php } else { echo '___aaf7'; } ?><?php echo $attgain[$clv]?><?php if (!defined('GEXIT_RETURN_JSON')) { ?>%</span><br>
</span></td>
<td class=b3 width=46>
<?php } else { echo '___aaf8'; } ?><?php if($upgradecost[$clv] !== -1) { if($skillpoint>=$upgradecost[$clv]) { ?>
...
<?php } else { echo '___aaf9'; } ?><?php } else { ?>
