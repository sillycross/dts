...
<?php } else { echo '___aaf1'; } ?><?php $clv=(int)\skillbase\skill_getvalue(44,'lvl'); $choice=(int)\skillbase\skill_getvalue(44,'choice'); $totlv = count($upgradecost)-1;  $redperc = min(50,$dmgreduction[$clv]*$def); ?>
...
每一点基础防御降低<span class="yellow"><?php } else { echo '___aahR'; } ?><?php echo $dmgreduction[$clv]?><?php if (!defined('GEXIT_RETURN_JSON')) { ?>%</span>固定伤害或爆炸伤害(最高50%)<br>
当前选择：降低你受到的<span class="yellow">
<?php } else { echo '___aahS'; } ?><?php if($choice==0) { ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?>固定伤害
<?php } else { echo '___aahT'; } ?><?php } else { ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?>爆炸伤害
<?php } else { echo '___aahU'; } ?><?php } ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?></span> 
...
<?php } else { echo '___aahV'; } ?><?php if($upgradecost[$clv] !== -1) { if($skillpoint>=$upgradecost[$clv]) { ?>
