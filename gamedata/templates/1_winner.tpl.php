...
<?php } else { echo '___aarq'; } ?><?php if($command == 'info') { include template('winnerinfo'); } elseif($command == 'news') { ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?><form method="post" name="info" action="winner.php">
<input type="submit" value="返回优胜列表">
<div align="left">
<?php } else { echo '___aarr'; } ?><?php echo $hnewsinfo?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?></div>
<input type="submit" value="返回优胜列表">
</form>
<?php } else { echo '___aars'; } ?><?php } else { include template('winnerlist'); } include template('footer'); ?>
