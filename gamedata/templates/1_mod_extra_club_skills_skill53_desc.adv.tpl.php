...
<?php } else { echo '___aagv'; } ?><?php include template('MOD_CLUBBASE_SKILLDESC_MID1'); ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?><td class=b3><span style="margin-top:8px; margin-bottom:8px; display:block;">
<?php } else { echo '___aaf1'; } ?><?php if(\skillbase\skill_getvalue(53,'l')=='0') { ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?>学习任意一个非战斗非限定技能
<?php } else { echo '___aagw'; } ?><?php \bubblebox\bubblebox_set_style('id:learntable;height:400;width:655;cancellable:1;margin-right:8;'); include template('MOD_BUBBLEBOX_START'); ?>
...
<?php } else { echo '___aagx'; } ?><?php \sklearn_util\get_skilllearn_table('\\skill53\\sklearn_checker53'); include template('MOD_BUBBLEBOX_END'); } else { $sk53_learnt=$clubskillname[(int)\skillbase\skill_getvalue(53,'l')]; ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?>你已经学习了技能“<span class="yellow"><?php } else { echo '___aagy'; } ?><?php echo $sk53_learnt?><?php if (!defined('GEXIT_RETURN_JSON')) { ?></span>”
<?php } else { echo '___aagz'; } ?><?php } ?>
<?php if (!defined('GEXIT_RETURN_JSON')) { ?></span></td>
<td class=b3 width=46>
<?php } else { echo '___aagA'; } ?><?php if(\skillbase\skill_getvalue(53,'l')=='0') { ?>
